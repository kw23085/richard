...
const Node: React.FC<Props> = ({ node, expanded, toggleNodeExpanded, nodeInfo }) => {
  return (
    <AccordionRoot
      elevation={3}
      expanded={expanded}
    ...
    >
      ...
      <AccordionDetails>
        {nodeInfo.map((info) => <Block blockNumber={info.blockNumber} blockContent={info.blockContent} key={info.blockNumber}></Block>)}
      </AccordionDetails>
    </AccordionRoot>
  );
};
